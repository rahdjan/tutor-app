...
function buildPrompt(topics: string[], subject: Subject): string {
  const topicList = topics.map((t) => `- ${t}`).join("\n");

  const intro =
    subject === "ENGLISH"
      ? "Ты извлекаешь упражнения по английскому языку из материалов репетитора (грамматические и лексические упражнения, задания на понимание текста, письменные задания). Материал — только текст/PDF/картинка, аудирования и говорения в нём не бывает — такие задания не извлекай."
      : "Ты извлекаешь математические задачи из материалов репетитора (домашние работы, варианты, сборники).";

  const statementHint =
    subject === "MATH"
      ? "полное условие; все формулы в LaTeX между $...$, выключные — между $$...$$"
      : "полное условие задания, как в материале (на английском или русском)";

  // LaTeX-инструкции нужны только математике — для английского они не
  // просто лишние, а вредные: сбивают модель на выдумывание формул.
  const formulaRules =
    subject === "MATH"
...
Верни СТРОГО JSON-объект вида {"tasks": [...]} без каких-либо пояснений и без markdown-ограждений. Формат каждой задачи в массиве tasks:
{
  ...
  "answer_type": "SHORT" — если ответ это число/слово/короткое выражение, иначе "DETAILED",
  "answer": "ответ, если он есть в материале или очевиден; иначе null",
  "solution": "решение или пояснение, если приведено в материале; иначе null",
  "difficulty": целое 1-5 (1 — самое простое, 3 — типовое, 5 — самое сложное),
...
${formulaRules}- Классификация: поле "topic" заполняй СТРОГО одним из заголовков списка ниже
  ...
  темы учебной программы, используется для точной сортировки задач при
  сборке рабочих листов, поэтому выбирай ближайшую по смыслу тему, а не
  самую общую.
...
export async function extractTasksWithAi(input: {
  ...
  const content: ContentPart[] = [
    { type: "text", text: buildPrompt(input.topics, input.subject) },
  ];
  if (input.file) {
